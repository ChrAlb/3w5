...
Map::Map(SharedContext* l_context, BaseState* l_currentState)
	:m_context(l_context), m_defaultTile(l_context), m_maxMapSize(64, 21), m_playerId(-1), m_currentState(l_currentState)
{
...
{
	auto itr = m_tileMap.find(ConvertCoords(coords.x, coords.y,1));
	m_tileMap.erase(itr);

	auto itr1 = m_tileSet.find(TileId);
	Tile* tile = new Tile();
	tile->m_properties = itr1->second;

	m_tileMap.emplace(ConvertCoords(coords.x, coords.y,1), tile).second;
}
...
void Map::SaveMap(const std::string& l_path)
{
	...
	std::string tileNr = "63";
	for (int i = 0; i < 64; i++)
		for (int j = 0; j < 22; j++)
		{

			Tile* tile = GetTile(i, j,1);
			int Nr = tile->m_properties->m_id;

			std::string line = "TILE ";

			line = line + Utils::inttostring(Nr);
			line = line + " ";

			line = line + Utils::inttostring(i);
			line = line + " ";

			line = line + Utils::inttostring(j);
			line = line + "\n";
...
void Map::drawTileSheet()
{
	...
	for (int tileId = 0; tileId < 64; tileId++)
	{

		auto itr = m_tileSet.find(tileId);
		sf::Sprite& sprite = itr->second->m_sprite;
		sprite.setPosition(col * Sheet::Tile_Size, row * Sheet::Tile_Size);
